 ...
 * @author mjra9
 */
public class GuestCheckOutController extends HttpServlet {
        ...
        if(!validator.validateEmail(email)){
            ...
            try{
                if(manager.getGuestUser(email) != null){
                    order.setUserID(manager.getUserID(email));
            }else{
                    manager.addGuestUser(email);
                    order.setUserID(manager.getUserID(email));
                }
            } catch (SQLException ex){
                System.out.println("Error in getting guest user in guest checkout " + " " +ex.getMessage());
            }
            try{
                if(manager.findOrder(order.getOrderID()) != null){
                    manager.updateOrder(order);
                ...
                System.out.println("error adding order to database " + order.getOrderID() + " " + ex.getMessage() + " " + ex.getStackTrace()); 
            }
            session.setAttribute("order", null);
            response.sendRedirect("index.jsp");
        }
